"""
Modified from the imagenet example provided by pytorch 
URL: https://github.com/pytorch/examples/tree/master/imagenet/main.py
"""
...
from model import ApplicationWithSSC 
...
## ssc hyper-parameters
parser.add_argument('--use-ssc', action='store_true', default=False)
parser.add_argument('--num-routings', type=int, default=3)
parser.add_argument('--num-groups', type=int, default=5)
parser.add_argument('--num-stains', type=int, default=2)
parser.add_argument('--group-width', type=int, default=3)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    if args.use_ssc:
        model = ApplicationWithSSC(args.arch, args.num_classes, args)
    else:
    ...
    if args.distributed:
        if args.weighted_sample:
            print('activate distributed weighted sampling')
            train_sampler = DistributedWeightedSampler(train_dataset, train_dataset.get_weights(), replacement=True)
        else:
    ...
    val_loader = torch.utils.data.DataLoader(
        PatchDataset(valdir, transforms.Compose([
    ...
    if args.use_ssc:
        prefix = prefix + '[ssc_r_{}_s{}_gn_{}_gw_{}]'.format(
            args.num_routings,
            args.num_stains,
            args.num_groups, 
            args.group_width
